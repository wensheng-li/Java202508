...
public class SystemManager {
	...
	private DisplayPersonByTypeStatus sts4;
	...
	public void statusSetting () {
		...
		sts1 = new ConsoleStatus (
				...
				"                                  メニュー¥n" +
				"  従業員検索 (S) ¥n" +
				"  従業員管理 (JI：追加　JU：更新　JD：削除) ¥n" +
				"  稼働状況管理 (KI：追加　KD：削除) ¥n" +
				"  終了 (X) ¥n" +
				"_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/¥n" ,
				"どの機能を実行しますか？¥n[S, JI, JU, JD, KI, KD, X]＞",
				false
				);
		// 軌道から”S”入力時の状態
		sts2 = new ConsoleStatus (
				"検索方法を指定してください。¥n" +
				"N ー＞氏名から検索　　Tー＞職種から検索¥n" +
				"Eー＞従業員検索終了（メニューに戻る）" ,
				"[N. T. E]＞",
				false
				);
		...
		sts4 = new DisplayPersonByTypeStatus (
				"",
				"E ー＞検索結果一覧終了（検索条件指定に戻る）［(従業員 ID), E］＞",
				false,
				plist,
				sts5
		);
		...
		sts12 = new DeleteWorkStatus (
				"",
				"この稼働情報を削除しますか？（Y　はい　N　いええ）[Y, N]＞",
				false,
				plist,wlist
		);
		
		// システムを終了する状態
		sts13 = new ExitStatus ("", "", true);
	...
	public void run() throws Exception {
		...
		while (!sts.getIsEndStatus()) {
			// 最初に出力するメッセージ
			sts.displayFirstMsg();
			// 次の状態に遷移すること促すためのメッセージ
			sts.displayPromptMsg();
			// キー入力を受付する
			cmd = sts.inputMessage();
			// キー入力されたコマンドによって、次の状態に遷移する
